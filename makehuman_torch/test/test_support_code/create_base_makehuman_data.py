# run from makehuman/makehuman directory
...
# Add makehuman directory to path BEFORE importing
makehuman_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", '..', 'makehuman')
)
if makehuman_dir not in sys.path:
    sys.path.insert(0, makehuman_dir)

# Local functions -----
# Add the `functions` directory to the Python path
functions_dir = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', '..', 'scripts', 'functions'
)
...
# ^ the assumption is these are fixed
import re
def create_faces_to_mask_vector(face_groups_names, face_to_group, human, keys = ["^joint-", "^helper-"]):
    ...
    return mask
# do we need to track masks for genatlia separately (through poxies? `def hasGenitals`?
my_face_mask = create_faces_to_mask_vector(face_groups_names, face_to_group, human, keys=["^joint-", "^helper-"])
...
# Get all modeling modifiers (these affect body shape)
modeling_modifiers = {}
modifiers_to_idx = {}
for m_idx, (mod_name, modifier) in enumerate(human._modifiers.items()):
    modeling_modifiers[mod_name] = modifier
    modifiers_to_idx[m_idx] = mod_name

print(f"  Found {len(modeling_modifiers)} total modifiers")

modifier_names = sorted(modeling_modifiers.keys())
num_target_modifiers = len(modifier_names)
num_vertices = len(base_mesh_vertices)

# Build 3D sparse COO tensor directly: (mod_idx, vert_idx, coord_idx)
# Accumulate indices as (mod_idx, vert_idx, coord_idx) and values
all_indices_list = []  # Will accumulate [mod_idx, vert_idx, coord_idx] triples
all_values_list = []
target_modifiers_delta_nnz_total = 0

for mod_idx, mod_name in enumerate(modifier_names):
    modifier = modeling_modifiers[mod_name]
    
    # Collect all affected vertices across all targets for this modifier
    mod_vertex_indices = []
    mod_translation_vectors = []
    
    # A modifier has a list of targets: (target_path, factors)
    if hasattr(modifier, 'targets') and modifier.targets:
        try:
            for target_path, factors in modifier.targets:
                # Load the target data using algos3d.getTarget
                ...
                if target and hasattr(target, 'verts') and hasattr(target, 'data'):
                    ...
                    translation_vectors = target.data  # Translation vectors for each affected vertex
                    
                    if len(vertex_indices) > 0:
                        mod_vertex_indices.extend(vertex_indices)
                        mod_translation_vectors.extend(translation_vectors)
        except Exception as e:
            print(f"    Warning: Could not load targets for {mod_name}: {e}")
    
    # Add this modifier's data to the 3D COO structure
    if len(mod_vertex_indices) > 0:
        try:
            mod_vertex_indices = np.array(mod_vertex_indices, dtype=np.int64)
            mod_translation_vectors = np.array(mod_translation_vectors, dtype=np.float32)
            
            # For each affected vertex, we need 3 entries (one per coordinate x, y, z)
            impacted_vertex_indices = np.repeat(mod_vertex_indices, 3)  # Repeat vertex indices 3 times
            coord_indices = np.tile([0, 1, 2], len(mod_vertex_indices))  # Cycle [0,1,2] for x,y,z
            mod_indices = np.full_like(impacted_vertex_indices, mod_idx)  # Fill with this modifier's index
            ...
            all_values_list.append(mod_translation_vectors.flatten())
            target_modifiers_delta_nnz_total += len(mod_translation_vectors.flatten())
            
        except Exception as e:
            print(f"    Warning: Could not process {mod_name}: {e}")
    
    if (mod_idx + 1) % 50 == 0:
        print(f"  Processed {mod_idx + 1}/{num_target_modifiers} modifiers...")
...
example_modifiers = np.zeros(num_target_modifiers, dtype=np.float32)
...
data_dict = {
    ...
    'face_mask': torch.tensor(my_face_mask, dtype=torch.bool, device=device),
...
output_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "test_data", "torch_model_data.pt")
)
